...
/* Signal handler invocation must be transparent for the code being
   ...
   The code below is qemu re-implementation of arch/zpu/kernel/signal.c */
...
struct target_sigcontext {
    ...
    uint64_t fpr[32];
    uint32_t fcsr;
}; /* cf. zpu-linux:arch/zpu/include/uapi/asm/ptrace.h */
...
static abi_ulong get_sigframe(struct target_sigaction *ka,
                              CPUZPUState* regs, size_t framesize)
{
    abi_ulong sp = regs->gpr[xSP];
    int onsigstack = on_sig_stack(sp);
...
static void setup_sigcontext(struct target_sigcontext *sc, CPUZPUState *env)
{
    int i;
...
static void setup_ucontext(struct target_ucontext* uc,
                           CPUZPUState* env, target_sigset_t *set)
{
    abi_ulong ss_sp = (target_ulong)target_sigaltstack_used.ss_sp;
    abi_ulong ss_flags = sas_ss_flags(env->gpr[xSP]);
    abi_ulong ss_size = target_sigaltstack_used.ss_size;
...
static void setup_rt_frame(int sig, struct target_sigaction *ka,
                           target_siginfo_t *info,
                           target_sigset_t *set, CPUZPUState *env)
{
    abi_ulong frame_addr;
    struct target_rt_sigframe *frame;
...
static void restore_sigcontext(CPUZPUState *env, struct target_sigcontext *sc)
{
    int i;
...
long do_rt_sigreturn(CPUZPUState *env)
{
    struct target_rt_sigframe *frame;
    abi_ulong frame_addr;
                                  ...
                                  struct emulated_sigtable *k)
{
    ...
    if (handler == TARGET_SIG_DFL) {
        ...
        || defined(TARGET_PPC64) || defined(TARGET_RISCV) || defined(TARGET_ZPU)
