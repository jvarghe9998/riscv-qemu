                     ...
                     int rm, uint64_t min)
{
    ...
    // check MSTATUS.FS
    tcg_gen_ld_tl(t0, cpu_env, offsetof(CPUZPUState, mstatus));
...
static void gen_jal(CPUZPUState *env, DisasContext *ctx, int rd,
                    target_ulong imm)
{
    target_ulong next_pc;
    ...
    if (!zpu_feature(env, ZPU_FEATURE_RVC)) {
        if ((next_pc & 0x3) != 0) {
            generate_exception_mbadaddr(ctx, ZPU_EXCP_INST_ADDR_MIS);
        }
    }
    if (rd != 0) {
...
static void gen_jalr(CPUZPUState *env, DisasContext *ctx, uint32_t opc,
...
static void gen_branch(CPUZPUState *env, DisasContext *ctx, uint32_t opc,
                       int rs1, int rs2, target_long bimm)
{
    ...
    if (!zpu_feature(env, ZPU_FEATURE_RVC) && ((ctx->pc + bimm) & 0x3)) {
        /* misaligned */
        generate_exception_mbadaddr(ctx, ZPU_EXCP_INST_ADDR_MIS);
        tcg_gen_exit_tb(0);
    } else {
        ...
        int rs1, int rs2, int rm)
{
    ...
    case OPC_RISC_FMV_X_S: {
            ...
            // check MSTATUS.FS
            tcg_gen_ld_tl(write_int_rd, cpu_env, offsetof(CPUZPUState, mstatus));
                      ...
                      int rd, int rs1, int csr)
{
    ...
    case OPC_RISC_ECALL:
        ...
        case 0x0: /* ECALL */
            ...
            generate_exception(ctx, ZPU_EXCP_U_ECALL);
            tcg_gen_exit_tb(0); /* no chaining */
            ctx->bstate = BS_BRANCH;
            break;
        case 0x1: /* EBREAK */
            generate_exception(ctx, ZPU_EXCP_BREAKPOINT);
            tcg_gen_exit_tb(0); /* no chaining */
            ctx->bstate = BS_BRANCH;
            break;
#ifndef CONFIG_USER_ONLY
...
static void decode_RV32_64C1(CPUZPUState *env, DisasContext *ctx)
{
    uint8_t funct3 = extract32(ctx->opcode, 13, 3);
...
static void decode_RV32_64C2(CPUZPUState *env, DisasContext *ctx)
{
    uint8_t rd, rs2;
    uint8_t funct3 = extract32(ctx->opcode, 13, 3);
...
static void decode_RV32_64C(CPUZPUState *env, DisasContext *ctx)
{
...
static void decode_RV32_64G(CPUZPUState *env, DisasContext *ctx)
{
    int rs1;
    int rs2;
    int rd;
