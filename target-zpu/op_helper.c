...
inline void QEMU_NORETURN do_raise_exception_err(CPUZPUState *env,
...
void helper_raise_exception(CPUZPUState *env, uint32_t exception)
{
    do_raise_exception_err(env, exception, 0);
}

void helper_raise_exception_debug(CPUZPUState *env)
{
    do_raise_exception_err(env, EXCP_DEBUG, 0);
}

void helper_raise_exception_mbadaddr(CPUZPUState *env, uint32_t exception,
        target_ulong bad_pc) {
    env->badaddr = bad_pc;
    do_raise_exception_err(env, exception, 0);
}
        ...
        target_ulong csrno)
{
    ...
    case CSR_MEDELEG: {
        target_ulong mask = 0;
        mask |= 1ULL << (ZPU_EXCP_INST_ADDR_MIS);
        mask |= 1ULL << (ZPU_EXCP_INST_ACCESS_FAULT);
        mask |= 1ULL << (ZPU_EXCP_ILLEGAL_INST);
        mask |= 1ULL << (ZPU_EXCP_BREAKPOINT);
        mask |= 1ULL << (ZPU_EXCP_LOAD_ADDR_MIS);
        mask |= 1ULL << (ZPU_EXCP_LOAD_ACCESS_FAULT);
        mask |= 1ULL << (ZPU_EXCP_STORE_AMO_ADDR_MIS);
        mask |= 1ULL << (ZPU_EXCP_STORE_AMO_ACCESS_FAULT);
        mask |= 1ULL << (ZPU_EXCP_U_ECALL);
        mask |= 1ULL << (ZPU_EXCP_S_ECALL);
        mask |= 1ULL << (ZPU_EXCP_H_ECALL);
        mask |= 1ULL << (ZPU_EXCP_M_ECALL);
...
inline target_ulong csr_read_helper(CPUZPUState *env, target_ulong csrno)
{
    ...
    case CSR_CYCLE:
        if (ctr_ok) {
            return cpu_zpu_read_instret(env);
        }
        break;
    case CSR_MINSTRET:
    case CSR_MCYCLE:
    ...
    case CSR_MCYCLEH:
#if defined(TARGET_ZPU32)
        return cpu_zpu_read_instret(env) >> 32;
#endif
        break;
    case CSR_MUCOUNTEREN:
...
void validate_csr(CPUZPUState *env, uint64_t which, uint64_t write)
{
    ...
    if (((write) && csr_read_only) || (env->priv < csr_priv)) {
        do_raise_exception_err(env, ZPU_EXCP_ILLEGAL_INST, env->pc);
    }
#endif
}

target_ulong helper_csrrw(CPUZPUState *env, target_ulong src,
        target_ulong csr)
{
...
target_ulong helper_csrrs(CPUZPUState *env, target_ulong src,
...
target_ulong helper_csrrc(CPUZPUState *env, target_ulong src,
        target_ulong csr, target_ulong rs1_pass)
{
    validate_csr(env, csr, rs1_pass != 0);
    uint64_t csr_backup = csr_read_helper(env, csr);
...
target_ulong helper_sret(CPUZPUState *env, target_ulong cpu_pc_deb)
{
    if (!(env->priv >= PRV_S)) {
        helper_raise_exception(env, ZPU_EXCP_ILLEGAL_INST);
    }
    ...
    if (!zpu_feature(env, ZPU_FEATURE_RVC) && (retpc & 0x3)) {
        helper_raise_exception(env, ZPU_EXCP_INST_ADDR_MIS);
    }
...
target_ulong helper_mret(CPUZPUState *env, target_ulong cpu_pc_deb)
{
    if (!(env->priv >= PRV_M)) {
        helper_raise_exception(env, ZPU_EXCP_ILLEGAL_INST);
    }
...
void helper_fence_i(CPUZPUState *env)
{
...
void helper_tlb_flush(CPUZPUState *env)
{
    ZPUCPU *cpu = zpu_env_get_cpu(env);
    tlb_flush(CPU(cpu), 1);
}
